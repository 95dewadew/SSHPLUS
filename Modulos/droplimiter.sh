...
if [ ! -f "$database" ]
then
	echo "Arquivo /root/usuarios.db tidak ditemukan"
	exit 1
fi
while true
do
	clear
	echo -e "\E[42;1;37m               LIMITE DROPBEAR                \E[0m"
    echo -e "\E[42;1;37m User                       Koneksi/limit \E[0m"
    while read usline
    do
		...
		if [ -z "$user" ] ; then
		    ...
		    if [ "$s3drop" -gt "$s2ssh" ]; then
		        echo -e "\E[41;1;37m Pengguna terputus karena melebihi batas! \E[0m"
		        while read line
		        do
